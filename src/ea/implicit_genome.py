...
def map_to_standard_mlp(codon_list, n_in, n_out, n_hidden=0, neurPerHid=0,
    ...
    if n_hidden <= 0:
        ...
        num_links = n_in * neurPerHid + neurPerHid * n_out + \
                    (n_hidden - 1) * (neurPerHid * neurPerHid)
    ...
    for idx in xrange(n_hidden):
        for j in xrange(neurPerHid):
            mapped_net.addModule(SigmoidLayer(1, \
                                        name="h" + str(idx) + "-" + str(j)))
    ...
    groupedCodons = [[x, [y[1] for y in codon_list if y[0] == x]] \
                                                for x in target_set]
    for l in xrange(num_links):
        ...
        if len(geneWeights) == 0:
            ...
            if polygene_strategy == "avg":
                weight = sum(geneWeights)/float(len(geneWeights))
            #other schemes?

        if n_hidden == 0:
            inNeur = mapped_net["i" + str(input_idx)]
            outNeur = mapped_net["o" + str(output_idx)]
            if input_idx == n_in - 1:
            ...
            if h == 0:
                inNeur = mapped_net["i" + str(input_idx)]
                outNeur = mapped_net["h" + str(h) + "-" + str(output_idx)]
                if input_idx == n_in - 1:
                    input_idx = 0
                    if output_idx == neurPerHid - 1:
                        output_idx = 0
                        h = h + 1
                    else:
                        output_idx = output_idx + 1
                ...
                if h < n_hidden:
                    inNeur = mapped_net["h" + str(h - 1) + "-" +\
                                                                str(input_idx)]
                    outNeur = mapped_net["h" + str(h) + "-" + str(output_idx)]
                    if input_idx == neurPerHid - 1:
                        input_idx = 0
                        if output_idx == neurPerHid - 1:
                            output_idx = 0
                            h = h + 1
        ...
        if "".join(genome[idx:]).startswith(start_codon):
            ...
            if j <= len(genome):
                ...
                if len(weight_string) == 0:
                    ...
                    codon_list.append([int("".join(tgt), 2),
...
###############################################################################
    ...
    if N_HID == 0:
        ...
        N_LINKS = N_IN * NEUR_HID_LAYER + \
                                        (N_HID - 1) * NEUR_HID_LAYER**2 + \
...
###############################################################################
    ...
    PROBLEM_ID = "MIN" #"RETAND"#"RETAND" # "MIN", "AND", "OR", "MAJ", "RETOR"
...
##########################Evolutionary algorithm###############################
    ...
    for i in xrange(MU):
        ...
        codons, pretty_genome_str = extract_codons(g, TGT_SIZE,\
                                                        START_CODON, END_CODON)
        net = map_to_standard_mlp(codons, N_IN, N_OUT, n_hidden=N_HID, \
                                                neurPerHid=NEUR_HID_LAYER)
        POPULATION.append(g)
        NETS.append(net)
        CODON_LIST.append(codons)
        PRETTY_GENOME_LIST.append(pretty_genome_str)
    ...
    for i, net in enumerate(NETS):
        fitness_pop[i] = evaluate(net, PROBLEM_DB, LABEL_DB,\
                                                        nb_instances=DB_SIZE)
    FITNESS_LOG = []
    FITNESS_LOG.append(fitness_pop)
    TIME_GEN_LOG = []
    NB_OF_CODONS_LOG = []
    #Total Time
    ...
    for ITERATION in xrange(1, NB_GENERATIONS + 1):
        ...
        parents = select(fitness_pop, LAMBDA)
        ...
        for i in parents:
            ...
            codons, pretty_genome_str = extract_codons(child, TGT_SIZE,\
                                                        START_CODON, END_CODON)
            net = map_to_standard_mlp(codons, N_IN, N_OUT, n_hidden=N_HID,\
                                                neurPerHid=NEUR_HID_LAYER)
            ...
            pretty_genomes_children.append(pretty_genome_str)
        ...
        survivors_idx = survive(fitness_pop + fitness_children, MU)
        print "Genomes"
        for g in POPULATION + children:
            print extract_codons(g, TGT_SIZE, START_CODON, END_CODON)[1]
            print "_____________________________________"
        POPULATION = [(POPULATION + children)[k] for k in survivors_idx]
        fitness_sel = [(fitness_pop + fitness_children)[k] \
        ...
        pretty_genomes_sel = [(PRETTY_GENOME_LIST +\
                            pretty_genomes_children)[k] for k in survivors_idx]
        print survivors_idx
        print "Fitness pop:"
        print fitness_pop
        print "Fitness child"
        ...
        for i, g in enumerate(POPULATION):
            codons_test.append(extract_codons(POPULATION[i], TGT_SIZE,\
        ...
        #print
        #print fitReEval
        #print fitness_pop == fitReEval

        #For codon counting only
        nb_codons = []
        for genome_string in POPULATION:
            codons, pretty_genome_str = extract_codons(genome_string,\
                                            TGT_SIZE, START_CODON, END_CODON)
